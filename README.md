...
- **安全保存**: auth.json 内容保存到系统凭据库，SQLite 仅保留账号元数据
...
- `codex_accounts.db` 是本工具管理的账号元数据仓库。
- 每个账号的 auth.json 敏感内容保存在系统凭据库中，不再明文写入 SQLite。
...
4. 选择备份文件并输入同一个密码，账号会恢复到新电脑的系统凭据库。
5. 需要使用哪个账号，再点击对应账号的「运行」。

备份文件和 `auth.json` 都包含敏感 token。请妥善保管备份密码，不要把备份文件提交到 Git，也不要发给别人。

不建议直接复制 `codex_accounts.db` 换电脑：数据库只保存账号元数据，缺少系统凭据库里的 auth.json 内容，复制后账号将无法直接切换或刷新额度。
